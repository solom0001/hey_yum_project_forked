    ...
    <div className="ContentWitdh md:gap-8 gap-2 ">
      ...
      <div className="gap-4 grid grid-cols-2 md:grid-cols-4 justify-center overflow-hidden!">
        {products.map((product) => (
          <Link key={product.id} href={`/detail/${product.id}?id=${product.id}`}>
            <div className="max-w-[250] w-full md:max-w-[400] flex flex-col gap-1">
              ...
              <h2 className="text-center font-lilita truncate">{product.name}</h2>
                ...
                {product.product_tags.map((tag, index) => (
                  <div key={tag.tags.name} className="flex w-fit truncate">
                    <p className="text-[10px]! truncate w-fit">
                      {tag.tags.name}
                      {index !== product.product_tags.length - 1 && <span className="text-[10px]!"> /</span>}
                    </p>
                  </div>
                ))}
              </div>
              <p className="text-center font-[400]! text-[1.5rem]! md:text-[1.6rem]! lg:text-[1.8rem]! uppercase mt-1">250 kr / kg</p>
            </div>
          </Link>
        ))}
      </div>
